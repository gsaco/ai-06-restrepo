...
[![Two Lean proofs checked](assets/lean.svg)](lean/ProofInterface.lean)
[![Partial formalization](assets/scope.svg)](lean/docs/FORMALIZATION_WORKING_MEMO.md)

# A guide to the paper, the checks, and the evidence

This repository studies **The Race Between Machine and Man** by Daron Acemoglu and Pascual Restrepo, using the **June 2017 revision of NBER Working Paper 22252**. It combines a presentation, a handwritten derivation, a small numerical check, and two supporting proofs in Lean.
...
- **Task-cost ranking:** Lean checks which factor is cheaper on either side of an assumed threshold. The proof requires positive factor prices and a positive, strictly increasing productivity function over all real task indices. That domain is stronger than the paper's active task interval. The threshold's existence and general equilibrium are not proved.
- **Wage-sign condition:** Lean checks when an algebraically defined wage response is positive. The decomposition is supplied as a definition. Lean does not derive it from production, household behavior, or market clearing.

A fresh check of the current declarations and proofs succeeded in the installed AppliedModelingLib environment. Both proofs reported only Lean's standard logical axioms: `propext`, `Classical.choice`, and `Quot.sound`. Neither contains `sorry`.

The earlier focused build and contributor fast check are recorded in the [working memo](lean/docs/FORMALIZATION_WORKING_MEMO.md). The complete source-to-Lean semantic audit, equilibrium proofs, and balanced-growth proofs remain unfinished. The `lean/` directory is a paper-folder snapshot, not a standalone Lake project with its own dependency configuration.
...
lean/
  PaperInterface.lean      Mathematical definitions and specifications
  ProofInterface.lean      Two supporting proofs
  MainTheorems.lean        Imported theorem layer
  Assumptions.lean         Assumption declaration layer
  status.json              Formalization status and audit configuration
  docs/                    Scope, planning, and working notes
  audit/                   Source-matching and audit records
```
